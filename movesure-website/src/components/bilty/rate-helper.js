...
export const HistoricalRateInfo = ({ 
  ...
  if (loading) {
    return (
      <div className="mt-2 flex items-center gap-2 px-3 py-2 border-2 border-blue-300 rounded-lg text-sm text-blue-700 bg-blue-50">
  ...
  return (
    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-semibold ${
      isDefault 
        ? 'bg-gradient-to-r from-purple-50 to-violet-50 text-purple-700 border-purple-300' 
      ...
      {!isDifferent && currentRate && (
        <span className={`font-bold ${isDefault ? 'text-purple-600' : 'text-green-600'} flex-shrink-0`}>✔️ लागू है</span>
      )}
    </div>
  );
};
