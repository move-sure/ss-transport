...
const BiltyList = ({ 
  ...
  return (
    <div className="space-y-4">
      {/* Challan Bilties Section - Show when challan is selected and has bilties */}
      {selectedChallan && filteredTransitBilties.length > 0 && (
        <div className="bg-white rounded-lg shadow-md border border-yellow-300 overflow-hidden">
          {/* Header */}
          <div className="bg-gradient-to-r from-yellow-500 to-amber-500 p-3">
            <div className="flex items-center justify-between">
              ...
              {!selectedChallan.is_dispatched && (
                <button
                  ...
                  disabled={selectedBilties.length === 0}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead className="bg-yellow-50">                <tr>
                  ...
                  <th className="px-2 py-1 text-left font-medium text-yellow-800 w-32">Consignee</th>
                  <th className="px-2 py-1 text-left font-medium text-yellow-800 w-24">Destination</th>
                  <th className="px-2 py-1 text-left font-medium text-yellow-800 w-16">Payment</th>
                  <th className="px-2 py-1 text-left font-medium text-yellow-800 w-12">Pkgs</th>
                  <th className="px-2 py-1 text-left font-medium text-yellow-800 w-16">Weight</th>
                  <th className="px-2 py-1 text-left font-medium text-yellow-800 w-20">Amount</th>
                  {!selectedChallan.is_dispatched && (
                    <th className="px-2 py-1 text-left font-medium text-yellow-800 w-12">Action</th>
                  )}
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-yellow-200">
                {filteredTransitBilties.map((bilty, index) => (                  <tr key={`transit-${bilty.id}-${bilty.bilty_type}`} className="hover:bg-yellow-50 transition-colors">
                    ...
                    <td className="px-2 py-1 text-gray-900 truncate max-w-32" title={bilty.consignor_name}>
                      {bilty.consignor_name}
                    </td>
                    <td className="px-2 py-1 text-gray-900 truncate max-w-32" title={bilty.consignee_name}>
                    ...
                    <td className="px-2 py-1 text-gray-900">{bilty.no_of_pkg}</td>
                    <td className="px-2 py-1 text-gray-900">{bilty.wt || 0}</td>
                    <td className="px-2 py-1 font-bold text-gray-900">₹{bilty.total}</td>
                    {!selectedChallan.is_dispatched && (
                      <td className="px-2 py-1">
                        <button
                          onClick={() => onRemoveBiltyFromTransit?.(bilty)}
      ...
      <div className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden">
        ...
        {/* Compact Available Bilties Table */}
        <div className="overflow-x-auto">
                ...
                <tr>
                  <th className="px-2 py-1 text-left w-8">
                    <button
                      onClick={handleSelectAll}
                      disabled={selectedChallan?.is_dispatched}
                      className="flex items-center gap-1 text-blue-800 font-medium disabled:opacity-50 text-xs"
                    >
                      {selectedBilties.length === filteredBilties.length && filteredBilties.length > 0 ? (
                        <CheckSquare className="w-3 h-3" />
                      ) : (
                  ...
                  <th className="px-2 py-1 text-left font-medium text-blue-800 w-32">Consignee</th>
                  <th className="px-2 py-1 text-left font-medium text-blue-800 w-24">Destination</th>
                  <th className="px-2 py-1 text-left font-medium text-blue-800 w-16">Payment</th>
                  <th className="px-2 py-1 text-left font-medium text-blue-800 w-12">Pkgs</th>
                  <th className="px-2 py-1 text-left font-medium text-blue-800 w-16">Weight</th>
                  <th className="px-2 py-1 text-left font-medium text-blue-800 w-20">Amount</th>
                </tr>
              </thead>
