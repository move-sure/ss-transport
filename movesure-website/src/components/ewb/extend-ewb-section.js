...
export default function ExtendEwbSection({ transitDetails, challanDetails }) {
  ...
  const fetchEwbAutoData = useCallback(async (ewbNumber) => {
    ...
    try {
      const url = `https://api.movesure.io//api/ewaybill?eway_bill_number=${ewbNumber}&gstin=${DEFAULT_USER_GSTIN}`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('https://api.movesure.io//api/extend-ewaybill', {
        method: 'POST',
