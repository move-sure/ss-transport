...
const CityAutocomplete = ({ 
  ...
  const handleInputChange = async (inputValue) => {
    ...
    // Search for cities
    await searchCities(upperValue);
    
    // Get filtered results for auto-fill logic
    const { data } = await supabase
      .from('cities')
      .select('id, city_name, city_code')
      .or(`city_name.ilike.%${upperValue}%,city_code.ilike.%${upperValue}%`)
      .order('city_name')
      .limit(20);
    ...
    if (filtered.length === 1 && upperValue.length > 2) {
    ...
    const exactMatch = filtered.find(city => 
      city.city_code.toLowerCase() === upperValue.toLowerCase()
    );
    
    if (exactMatch && upperValue.length >= 2) {
      console.log('🎯 Exact city code match found:', exactMatch.city_name);
      setTimeout(() => {
