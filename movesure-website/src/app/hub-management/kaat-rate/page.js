...
export default function KaatRatePage() {
  ...
  const loadInitialData = async () => {
    try {
      ...
      const enrichedRates = ratesData.map(rate => {
        ...
        return {
          ...rate,
          transport: transport ? { id: transport.id, transport_name: transport.transport_name, city_name: transport.city_name, gst_number: transport.gst_number } : null,
          destination_city: city ? { id: city.id, city_name: city.city_name, city_code: city.city_code } : null,
        };
      });
      ...
      const activeRates = enrichedRates.filter(r => r.is_active);
      const uniqueTransportsCount = new Set(enrichedRates.filter(r => r.transport_id).map(r => r.transport_id)).size;
      const uniqueCities = new Set(enrichedRates.map(r => r.destination_city_id)).size;

      setStats({ totalRates: enrichedRates.length, activeRates: activeRates.length, uniqueTransports: uniqueTransportsCount, uniqueCities });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
        ...
        <div className="flex items-center justify-center min-h-[80vh]">
          <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-2xl p-12 border border-white/50 max-w-md w-full mx-4 text-center">
            <Loader2 className="animate-spin h-16 w-16 text-emerald-600 mx-auto mb-6" />
            <h3 className="text-2xl font-bold bg-gradient-to-r from-emerald-900 to-teal-800 bg-clip-text text-transparent mb-2">Loading Kaat Rates</h3>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
        ...
        <div className="container mx-auto px-6 py-16">
          <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-2xl p-10 border border-red-200 max-w-lg mx-auto text-center">
            <AlertCircle className="w-12 h-12 text-red-500 mx-auto mb-4" />
            <h2 className="text-2xl font-bold text-gray-900 mb-3">Unable to Load Data</h2>
            <p className="text-gray-600 mb-6">{error}</p>
            <button onClick={loadInitialData} className="px-8 py-4 bg-gradient-to-r from-red-600 to-rose-600 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all">
              Try Again
            </button>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <div className="container mx-auto px-4 py-6 max-w-[1800px]">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <p className="text-gray-600 text-sm mt-1">Manage transport hub rates and pricing across all destinations</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
          <div className="group relative bg-white rounded-xl shadow-md hover:shadow-xl p-4 border-2 border-gray-100 hover:border-emerald-300 transition-all duration-300 overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-2">
                <div className="bg-gradient-to-br from-emerald-500 to-teal-600 p-2 rounded-lg shadow-md"><DollarSign className="w-5 h-5 text-white" /></div>
                <div className="text-right">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Total Rates</p>
                  <p className="text-2xl font-bold bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">{stats.totalRates}</p>
                </div>
              </div>
          ...
          <div className="group relative bg-white rounded-xl shadow-md hover:shadow-xl p-4 border-2 border-gray-100 hover:border-blue-300 transition-all duration-300 overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-2">
                <div className="bg-gradient-to-br from-blue-500 to-indigo-600 p-2 rounded-lg shadow-md"><Package className="w-5 h-5 text-white" /></div>
                <div className="text-right">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Active</p>
                  <p className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">{stats.activeRates}</p>
                </div>
              </div>
          ...
          <div className="group relative bg-white rounded-xl shadow-md hover:shadow-xl p-4 border-2 border-gray-100 hover:border-purple-300 transition-all duration-300 overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-2">
                <div className="bg-gradient-to-br from-purple-500 to-pink-600 p-2 rounded-lg shadow-md"><Users className="w-5 h-5 text-white" /></div>
                <div className="text-right">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Transports</p>
                  <p className="text-2xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">{stats.uniqueTransports}</p>
                </div>
              </div>
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="text-right">
                  ...
                  <p className="text-2xl font-bold bg-gradient-to-r from-amber-600 to-orange-600 bg-clip-text text-transparent">{stats.uniqueCities}</p>
                </div>
              </div>
      ...
      <AddKaatModal isOpen={showAddModal} onClose={() => setShowAddModal(false)} cities={cities} onSuccess={handleRateAdded} />

      <KaatPDFSettingsModal isOpen={showPDFModal} onClose={() => setShowPDFModal(false)} cities={cities} transports={transports} onGenerate={handleGeneratePDF} />

      <KaatPDFPreview
        isOpen={showPDFPreview}
        onClose={() => { setShowPDFPreview(false); if (pdfUrl) { URL.revokeObjectURL(pdfUrl); setPdfUrl(null); } }}
        pdfUrl={pdfUrl}
