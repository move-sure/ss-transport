...
const ChallanBiltyList = ({ bilties, transitDetails, loading, branches = [], onComplaintCreated, onStatusUpdate, user, cities = [], challanNo }) => {
  ...
  const fetchComplaints = async () => {
    try {
      const grNumbers = bilties.map(b => b.gr_no);
  ...
  const getComplaintStatus = (grNo) => {
    return complaints.find(c => c.gr_no === grNo);
  };
  ...
  const getTransitStatus = (grNo) => {
    const complaint = getComplaintStatus(grNo);
    if (!complaint) return { label: 'Unknown', color: 'bg-slate-100 text-slate-700' };

    if (complaint.is_delivered_at_destination) {
      return { label: 'Delivered', color: 'bg-emerald-100 text-emerald-700' };
    }
    if (complaint.is_out_for_delivery) {
      return { label: 'Out for Delivery', color: 'bg-blue-100 text-blue-700' };
    }
    if (complaint.is_out_for_delivery_from_hub) {
      return { label: 'Out from Hub', color: 'bg-indigo-100 text-indigo-700' };
    }
    if (complaint.is_at_hub) {
      return { label: 'At Hub', color: 'bg-amber-100 text-amber-700' };
    }
        ...
        {filteredBilties.length === 0 ? (
          ...
          <div className="space-y-2">
            {filteredBilties.map((bilty) => {
              ...
              const complaint = getComplaintStatus(bilty.gr_no);

              return (
                <div
                  key={bilty.id}
                  className="rounded-lg border border-slate-200 bg-white p-1.5 shadow-sm transition hover:border-indigo-300 hover:shadow"
                >
                  <div className="mb-1 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      <div>
                        <p className="text-xs font-black text-slate-900">GR-{bilty.gr_no}</p>
                        <p className="text-[9px] text-slate-500">{formatDate(bilty.bilty_date)}</p>
                      </div>
                    </div>
                    <div className="flex flex-wrap gap-0.5">
                      <span
                        className={`rounded-full border px-1 py-0.5 text-[8px] font-bold uppercase ${getPaymentModeColor(
                          bilty.payment_mode
                        )}`}
                      >
                      ...
                      <span
                        className={`rounded-full px-1 py-0.5 text-[8px] font-bold uppercase ${
                      ...
                      <span className={`rounded-full px-1 py-0.5 text-[8px] font-bold uppercase ${transitStatus.color}`}>
                        {transitStatus.label}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1 text-xs">
                    <div className="grid gap-1 sm:grid-cols-2">
                      <div className="rounded border border-indigo-100 bg-indigo-50/50 px-1.5 py-0.5">
                        <p className="text-[7px] font-bold uppercase tracking-wider text-indigo-600 mb-0.5">Consignor</p>
                        <div className="flex items-center justify-between gap-1">
                          <p className="truncate text-[10px] font-bold text-slate-900 flex-1">{bilty.consignor_name || 'N/A'}</p>
                          {bilty.consignor_number && (
                            <a
                              href={`tel:${bilty.consignor_number}`}
                              className="flex items-center gap-0.5 text-[8px] font-semibold text-indigo-600 hover:text-indigo-800 shrink-0"
                            >
                      ...
                      <div className="rounded border border-purple-100 bg-purple-50/50 px-1.5 py-0.5">
                        <p className="text-[7px] font-bold uppercase tracking-wider text-purple-600 mb-0.5">Consignee</p>
                        <div className="flex items-center justify-between gap-1">
                          <p className="truncate text-[10px] font-bold text-slate-900 flex-1">{bilty.consignee_name || 'N/A'}</p>
                          {bilty.consignee_number && (
                            <a
                              href={`tel:${bilty.consignee_number}`}
                              className="flex items-center gap-0.5 text-[8px] font-semibold text-purple-600 hover:text-purple-800 shrink-0"
                            >
                              <Phone className="h-2 w-2" />
                    ...
                    {bilty.to_city_id && (
                      <div className="rounded border border-pink-100 bg-pink-50/50 px-1.5 py-0.5">
                        <p className="text-[7px] font-bold uppercase tracking-wider text-pink-600 mb-0.5">Destination</p>
                        <p className="text-[10px] font-bold text-pink-900">{getCityName(bilty.to_city_id)}</p>
                      </div>
                    )}

                    <div className="flex items-center gap-1.5 rounded border border-slate-200 bg-white px-1.5 py-0.5">
                      <span className="text-[10px] font-semibold text-slate-900">{bilty.wt || 0}KG</span>
                      <span className="text-slate-400 text-[10px]">•</span>
                      <span className="text-[10px] font-semibold text-slate-900">{bilty.no_of_pkg || 0}PKG</span>
                      <span className="text-slate-400 text-[10px]">•</span>
                      <span className="text-[10px] font-bold text-emerald-700">₹{bilty.total || 0}</span>
                      <span className="text-slate-400 text-[10px]">•</span>
