...
export default function PohonchPrintPage() {
    ...
    try {
      ...
      if (f.length) {
        ...
        if (transportIds.length > 0) {
          ...
          if (unique.length === 1) { setSelectedTransport(unique[0]); setTransportSearch(`${unique[0].transport_name}${unique[0].gst_number ? ' | GST: ' + unique[0].gst_number : ''}`); }
          else { setSelectedTransport(null); setTransportSearch(''); }
        }
      }
      await fetchBiltyDetails(f);
      const em = {}; f.forEach(k => { em[k.challan_no] = true; }); setSbExpandedChallans(em);
  ...
  const handleSavePohonch = async () => {
    ...
    try {
      ...
      const gstQ = selectedTransport.gst_number
  ...
  const handleOpenCreateModal = async () => {
    setCreateResult(null); setCreatePrefix('');
    if (selectedTransport) {
      try {
        ...
        const { data } = await q;
        if (data?.length) { const parsed = detectLastUsedPrefix(data[0].pohonch_number); if (parsed?.prefix) setCreatePrefix(parsed.prefix); }
      } catch (e) { /* ignore */ }
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-teal-50 to-emerald-50">
      ...
      <div className="w-full mx-auto px-4 py-6">
        ...
        {/* Header — back to Hub Management */}
        <div className="mb-6 bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/50 p-6">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-4">
              <button onClick={() => router.push('/hub-management')} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
